...
const btnSaltaRiepilogo = document.getElementById('btnSaltaRiepilogo');
const btnTornaHome = document.getElementById('btnTornaHome');
...
btnCarica.addEventListener('click', () => {
    ...
    try {
        ...
        let testoFulito = testo.replace(/\[cite_start\]/g, '');
        testoFulito = testoFulito.replace(/\[cite:\s*\d+(?:\s*,\s*\d+)*\]/g, '');
        
        // Parsing con tolleranza agli errori
        domande = JSON.parse(testoFulito);
...
// Genera tutte le immagini in background IN PARALLELO
async function generaImmaginiInBackground() {
    // Crea un array di promesse per tutte le immagini
    const promesseImmagini = domande.map((domanda, i) => {
        // Salta se l'immagine è già stata generata
        if (immaginiGenerate[i]) {
            return Promise.resolve({ index: i, url: immaginiGenerate[i] });
        }
        
        return generaImmagineAI(domanda.domanda, i)
            .then(url => {
                immaginiGenerate[i] = url;
                ...
                if (i === indiceDomandaCorrente && immaginiGenerate[i]) {
                    const immagineAI = document.getElementById('immagineAI');
                    immagineAI.src = immaginiGenerate[i];
                }
    ...
    Promise.all(promesseImmagini)
        ...
        .catch(error => {
            console.error('Errore nella generazione parallela delle immagini:', error);
        });
}

// Riprende la generazione delle immagini dal punto dove era rimasta IN PARALLELO
async function riprendiGenerazioneImmagini() {
    // Crea un array di promesse solo per le immagini non ancora generate
    const promesseImmagini = domande
        .map((domanda, i) => {
            // Salta se l'immagine è già stata generata
            if (immaginiGenerate[i]) {
                return null;
    ...
    // Se non ci sono immagini da generare, esci
    if (promesseImmagini.length === 0) {
        const contatore = document.getElementById('contatoreImmagini');
        contatore.style.display = 'none';
        return;
    }
    ...
    Promise.all(promesseImmagini)
        ...
        .catch(error => {
            console.error('Errore nella ripresa della generazione parallela:', error);
        });
}
...
async function generaImmagineAI(testoDomanda, seed = null) {
    try {
        ...
        return `https://via.placeholder.com/800x450/4CAF50/ffffff?text=${encodeURIComponent('Quiz')}`;
    }
}
...
// Salta al riepilogo
btnSaltaRiepilogo.addEventListener('click', () => {
    indiceDomandaCorrente = domande.length; // Salta alla fine
    mostraRiepilogo();
    salvaStato();
});
